...
int main()
{
    ...
    // to remove \n and replace with \0
    int iteratorI;
    for (iteratorI = 0; input[iteratorI] != NULL_CHAR; iteratorI++)
    {
        if (input[iteratorI] == '\n')
        {
            input[iteratorI] = NULL_CHAR;
            break;
        }
    }
...
{
    if (operator == DIVIDE)
        return 3;               //first Highest priority
    if (operator == MULTIPLY)
        return 2;             //second high priority
    if (operator == ADD)
        return 1;           
...
int validateInput(char input[])
{
    ...
    for (int iteratorI = 0; input[iteratorI] != NULL_CHAR; iteratorI++)
    {
...
void removeBlankSpaces(char input[])
{
    ...
    while (input[iteratorI] != NULL_CHAR)
    {
        if (input[iteratorI] != ' ')
        {
    ...
    input[index] = NULL_CHAR; // Terminate cleaned string
}
